...
const int lulluby_melody[] PROGMEM = {
...
void SpeakerSequencer::setup()
{
  ...
  curnoteidx = 0;
  songptr = zelda_melody;
  tempo = 88;
  totalnotes =sizeof(zelda_melody)/sizeof(zelda_melody[0])/2; 
...
void SpeakerSequencer::update()
{
  ...
  if ( playing )
  {
    if ( millis() >= nextnote)
    {
      ...
      //divider = songptr[curnoteidx + 1];
      divider = pgm_read_word_near(songptr+curnoteidx + 1);
      if (divider > 0) {
      ...
      //tone(SPEAKER_PIN, songptr[curnoteidx], noteDuration*0.9);
      tone(SPEAKER_PIN, pgm_read_word_near(songptr+curnoteidx), noteDuration*0.9);
  
      // Wait for the specief duration before playing the next note.
      nextnote = millis() + noteDuration;
      curnoteidx += 2;
      if (curnoteidx >= totalnotes)
      {
        curnoteidx = 0;
      } 
    }
...
void SpeakerSequencer::Stop()
{
  playing = false;
  noTone(SPEAKER_PIN);
}

void SpeakerSequencer::setSong(int song)
{
  switch(song)
  {
    case 0:
    ...
    case 1:
      curnoteidx = 0;
      songptr = lulluby_melody;
      tempo = 108;
      totalnotes =sizeof(lulluby_melody)/sizeof(lulluby_melody[0])/2; 
      wholenote = (60000 * 4) / tempo;
      break;
    case 2:
      curnoteidx = 0;
      songptr = doom_melody;
      tempo = 225;
      totalnotes =sizeof(doom_melody)/sizeof(doom_melody[0])/2; 
      wholenote = (60000 * 4) / tempo;
      break;
    case 3:
      curnoteidx = 0;
